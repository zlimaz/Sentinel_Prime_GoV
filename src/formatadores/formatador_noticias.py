...
def format_news_thread(article):
    ...
    tweet1 = textwrap.shorten(header, width=(MAX_TWEET_LENGTH - len(footer) - len(hashtags)), placeholder="...") + footer + hashtags
    ...
    tweet2_content = textwrap.shorten(summary, width=available_space, placeholder="... (leia mais no link)")
    ...
    logging.info(f"Thread formatada para a notícia: '{title}'")
    return thread
...
if __name__ == '__main__':
    ...
    sample_article = {
        'title': "Comissão aprova projeto que altera regras de aposentadoria para servidores públicos em nova sessão extraordinária",
        'link': 'https://www.camara.leg.br/noticias/exemplo-de-link',
        'summary': 'O texto, que segue para análise do Plenário, estabelece um novo cálculo para o benefício e regras de transição. A proposta foi alvo de intenso debate entre os parlamentares, com discussões que se estenderam por toda a tarde. A oposição criticou a velocidade da tramitação.'
    }
